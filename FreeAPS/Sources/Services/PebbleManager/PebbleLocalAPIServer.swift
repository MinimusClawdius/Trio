...
final class PebbleLocalAPIServer {
    ...
    private func handleBolusRequest(_ body: String?) -> (Int, String, String) {
        ...
        let resp = "{\"status\":\"pending_confirmation\",\"commandId\":\"\(command.id)\",\"message\":\"Confirm \(String(format: "%.2f", units))U bolus on iPhone\",\"type\":\"bolus\"}"
        return (202, "application/json", resp)
    }
    ...
    private func handleCarbRequest(_ body: String?) -> (Int, String, String) {
        ...
        let resp = "{\"status\":\"pending_confirmation\",\"commandId\":\"\(command.id)\",\"message\":\"Confirm \(String(format: "%.0f", grams))g carbs on iPhone\",\"type\":\"carbEntry\"}"
        return (202, "application/json", resp)
    }
    ...
    private func extractMethod(from request: String) -> String {
        ...
        guard let first = lines.first else { return "GET" }
        return first.components(separatedBy: " ").first ?? "GET"
    }
